...
export class PeliculaEditComponent {
  ...
  constructor(
    ...
    if (this.idRuta != null) {
      this.peliApi.getById(+this.idRuta).subscribe((data) => {
        this.peliculaEdit = data;
      });
    }
  ...
  handleSaveClick() {
    if (this.peliForm.valid) {
      const movie = this.peliForm.value;

      if (this.idRuta == null) {
        this.peliApi.Insert(movie).subscribe({
          next: (peli) => {
        ...
        this.peliApi.Update(movie, +this.idRuta).subscribe({
          next: (peli) => {
            window.location.replace('http://localhost:4200/');
          },
          error: (peli) => {
